...
.factory('Bike', ['railsResourceFactory', '$cacheFactory', 'FilterService', function(railsResourceFactory, $cacheFactory, FilterService) {

  var bike = railsResourceFactory({url: '/bikes', name: 'bike'});
    ...
    if(cachedBikes) {
      ...
      this.query().then(function(data) {
        var bikes = _.map(data, function(b) {
          ...
          b.attrsString = _.map(attrs, function(attr){ return attr.toString().split(" "); }).join("").replace(/\s|,/g, '').toLowerCase();
  ...
  FilterService.prototype = {
    ...
    getPriceTermObj: function(filterTermsString) {
      ...
      if(priceTerm) {
        ...
        var greaterThan = (filterTermsString.match(/(greater than)|(cost more than)|(more than)/))? true : false
