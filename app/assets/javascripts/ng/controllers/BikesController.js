angular.module('bike.controllers').controller('BikesCtrl', ['$scope', '$http', '$routeParams', '$location','Bike', 'FilterService', 'BikeImages','$resource', function($scope, $http, $routeParams, $location, Bike, FilterService, BikeImages, $resource){

	Bike.query().then(function(data) {
		$scope.bikes = _.map(data, function(bike) {
      var attrs = [bike.model, bike.brand, bike.year, bike.kind, bike.price, bike.groupset, bike.material]
      bike.attrsString = _.map(attrs, function(attr){ return attr.toString().split(" ") }).join("").replace(/\s|,/g, '').toLowerCase()
      bike.filterService = new FilterService(bike);
      return bike;
    })
	})

	var bikeId = parseInt($routeParams.bikeId)
	
	if(bikeId){
		Bike.get(bikeId).then(function(response){
			window.bike = $scope.bike = response
		})
	}
